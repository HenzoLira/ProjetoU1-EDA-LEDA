...
public class InsertionSort {
	...
	public static void insertionSort(int[] array) {
		...
		for(int j = 1; j < n; j++) {			// começa no segundo elemento e o considera a chave para inserção
			int key = array[j];					// armazena o valor a ser inserido na porção ordenada.
			...
			while (i>=0 && array[i] > key) {	// i >= 0 garante que o índice não saia dos limites (não tenta acessar um índice negativo)
